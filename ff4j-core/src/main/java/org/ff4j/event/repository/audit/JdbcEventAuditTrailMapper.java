 ...
 */
public class JdbcEventAuditTrailMapper extends JdbcMapper implements EventMapper < PreparedStatement, ResultSet> {
  
    public JdbcEventAuditTrailMapper(Connection sqlConn, JdbcQueryBuilder qbd) {
        super(sqlConn, qbd);
    }
    
    public PreparedStatement buildSelectStatement(AuditTrailQuery query) {
        try {
            StringBuilder sqlQuery = new StringBuilder(queryBuilder.sqlSelectAllAudit())
                    .append(" WHERE ")
            ...
            if (!query.getFilteredValues().isEmpty()) {
                sqlQuery.append(" AND ")
                        .append(AuditTrailColumns.SCOPE.colname() + " IN")
                        .append(toStringListWithComma(query.getFilteredScopes()));
            }
            if (!query.getFilteredValues().isEmpty()) {
                sqlQuery.append(" AND ")
                        .append(AuditTrailColumns.VALUE.colname() + " IN")
    ...
    public PreparedStatement buildPurgeStatement(AuditTrailQuery query) {
        try {
    ...
    public Event mapFromRepository(ResultSet rs) {
        try {
            ...
            eventBuilder.scope(Scope.valueOf(rs.getString(AuditTrailColumns.SCOPE.colname())));
