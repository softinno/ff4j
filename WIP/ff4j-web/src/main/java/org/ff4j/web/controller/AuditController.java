...
public class AuditController extends AbstractController {
     ...
     *      curren query
     */
    private void createPage(WebContext ctx, AuditTrailQuery query) {
        ctx.setVariable(KEY_TITLE, "AuditTrail");
        query.getFrom().ifPresent(from -> {
            ctx.setVariable("from", SDFSLOT.format(new Date(from)));
        });
        query.getTo().ifPresent(to -> {
            ctx.setVariable("to",  SDFSLOT.format(new Date(to)));
        });
        ctx.setVariable(WebConstants.KEY_AUDITTRAIL, ff4j.getAuditTrail().search(query));
    }
