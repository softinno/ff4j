...
public class FF4jServlet extends HttpServlet {
    ...
    private void initializeFF4J(ServletConfig servletConfig) {
	    ...
	    try {
	        ...
	        LOGGER.info("ff4j context has been successfully initialized - {} feature(s)", ff4jProvider.getFF4j().getRepositoryFeatures().count());
	    ...
	    LOGGER.debug("Servlet has been initialized and ff4j store in session with {} ", ff4j.getRepositoryFeatures().count());
