...
public class FeatureTagEnable extends AbstractFeatureTag {
    ...
    protected boolean eval(FF4j ff4j, PageContext jspContext) {
        FF4jContext ctx = new FF4jContext(ff4j);
        if (isShareHttpSession()) {
            ctx.put("LOCALE", pageContext.getRequest().getLocalName());
            ...
            for (Map.Entry<String,String[]> param : parameters.entrySet()) {
                ...
                if (innerParams != null) {
                    ...
                    ctx.put(param.getKey(), expression.substring(0, expression.length() - 1));
                }
            }
        }
        return ff4j.check(getFeatureid(), ctx);
    }
