...
public class FeatureUsageController extends AbstractController {
    ...
    throws Exception {
        createPage(ctx,  new AuditTrailQuery());
     ...
     *      curren query
     */
    private void createPage(WebContext ctx, AuditTrailQuery eqd) {
        ctx.setVariable(KEY_TITLE, "Feature Usage");
        eqd.getFrom().ifPresent(from -> {
           ctx.setVariable("from", SDFSLOT.format(new Date(from)));
           ctx.setVariable("fromJS", SDF.format(new Date(from)));
           
        });
        eqd.getTo().ifPresent(to -> {
            ctx.setVariable("to", SDFSLOT.format(new Date(to)));
            ctx.setVariable("toJS", SDF.format(new Date(to)));
         });
    }
