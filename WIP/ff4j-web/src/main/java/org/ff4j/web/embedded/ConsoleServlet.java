...
public class ConsoleServlet extends HttpServlet {
        ...
        if (ff4j == null) {
            ...
            try {
                ...
                LOGGER.info("ff4j context has been successfully initialized - {} feature(s)", 
                        ff4jProvider.getFF4j().getRepositoryFeatures().count());
            ...
            LOGGER.debug("Servlet has been initialized and ff4j store in session with {} ", 
                    ff4j.getRepositoryFeatures().count());
        } else {
        ...
        try {
            ...
            if (operation != null && !operation.isEmpty()) {
                ...
                if ((featureId != null) && (!featureId.isEmpty())) {
                    ...
                    if (getFf4j().getRepositoryProperties().exists(featureId)) {
    ...
    throws IOException {
        String message = null;
        PropertyRepository pRepo = getFf4j().getRepositoryProperties();
        if (OP_RMV_PROPERTY.equalsIgnoreCase(operation)) {
            pRepo.deleteProperty(uid);
        ...
        if (OP_DELETE_FIXEDVALUE.equalsIgnoreCase(operation)) {
            ...
            Property<?> ap =  pRepo.read(uid);
            ap.getFixedValues().ifPresent(fixedValues -> fixedValues.remove(fixedValue));
            pRepo.save(ap);
        }
    ...
    throws IOException {
        String message = null;
        FeatureRepository fRepo = getFf4j().getRepositoryFeatures();
                    ...
                    if (OP_EDIT_FEATURE.equalsIgnoreCase(operation)) {
                        ...
                        if (groupName != null && !groupName.isEmpty()) {
                            ...
                            if (OP_ENABLE.equalsIgnoreCase(operationGroup)) {
                                getFf4j().getRepositoryFeatures().toggleOnGroup(groupName);
                                ...
                                getFf4j().getRepositoryFeatures().toggleOffGroup(groupName);
