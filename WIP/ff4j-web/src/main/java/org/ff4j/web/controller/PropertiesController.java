...
public class PropertiesController extends AbstractController {
    ...
    throws IOException {
        ...
        if (OP_CREATE_PROPERTY.equalsIgnoreCase(operation)) {
            ...
            if (!Util.hasLength(propertyName)) {
                ...
                getFf4j().getRepositoryProperties().delete(propertyName);
            ...
            Set< String> propertiesNames = getFf4j().getRepositoryProperties().getPropertyNames().collect(Collectors.toSet());
            if (propertiesNames.contains(newName)) {
                msgType = "warning";
                msg = "Cannot rename " + propertyName + " to " + newName + " : it already exists";
            } else {
                Property<?> oldProp = getFf4j().getRepositoryProperties().read(propertyName);
                Property<?> newProp = PropertyFactory.createProperty(NEW_NAME, 
                        oldProp.getClassName(), oldProp.getValueAsString());
                if (oldProp.getFixedValues().isPresent()) {
                    for(Object fixedValue : oldProp.getFixedValues().get()) {
                        newProp.add2FixedValueFromString(fixedValue.toString());
                    }
                }
                if (oldProp.getDescription().isPresent()) {
                    newProp.setDescription(oldProp.getDescription().get());
                }
                getFf4j().getRepositoryProperties().delete(propertyName);
                getFf4j().getRepositoryProperties().save(newProp);
                msg = "Property " + propertyName + " has been renamed to " + newName;
            }
            
        } else if (OP_COPY_PROPERTY.equalsIgnoreCase(operation)) {
            String newName = req.getParameter(NEW_NAME);
            Set< String> propertiesNames = getFf4j()
	...
	throws IOException {
        ...
        if (Util.hasLength(operation) && Util.hasLength(propertyName) && 
                getFf4j().getRepositoryProperties().exists(propertyName)) {
           
            if (OP_DELETE_FIXEDVALUE.equalsIgnoreCase(operation)) {
                String fixedValue = req.getParameter(PARAM_FIXEDVALUE);
                Property<?> ap = getFf4j().getRepositoryProperties().read(propertyName);
                // Need to convert back to object to use the remove()
                if (ap.getFixedValues().isPresent()) {
                    ap.getFixedValues().get().remove(ap.fromString(fixedValue));
                }
                getFf4j().getRepositoryProperties().save(ap);
    ...
    private void renderPage(WebContext ctx) {
        ...
        Map<String, Property<?>> mapOfProperties = ff4j.getRepositoryProperties().findAllAsMap();
