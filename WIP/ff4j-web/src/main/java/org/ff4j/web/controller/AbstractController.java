...
public abstract class AbstractController {
    ...
    protected AuditTrailQuery buildQuery(HttpServletRequest req) {
        AuditTrailQuery edf = new AuditTrailQuery();
        try {
            Date from = SDFSLOT.parse(req.getParameter("slotfrom"));
            Date to   = SDFSLOT.parse(req.getParameter("slotto"));
            edf = new AuditTrailQuery(from.getTime(),to.getTime());
        }  catch(ParseException pe) {}
        return edf;
    }
    
    protected FeatureRepository getFeatureRepository() {
        return getFf4j().getRepositoryFeatures();
    }
    
    protected Set < String > getFeatureNames() {
        return getFf4j().getRepositoryFeatures().listFeatureNames().collect(Collectors.toSet());
    }
