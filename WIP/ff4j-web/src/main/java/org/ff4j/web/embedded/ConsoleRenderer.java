...
public final class ConsoleRenderer {
    ...
    private static final String renderPropertiesRows(FF4j ff4j, HttpServletRequest req) {
        ...
        ff4j.getRepositoryProperties().findAll().forEach(p -> {
            ...
            p.getDescription().ifPresent(desc -> sb.append(" tooltip=\"" + desc + "\""));
            sb.append(">");
            // Unique identifier
            sb.append(renderValue(p.getUid(), 50));
            sb.append("</a>");
            // Value
            sb.append("</td><td>");
            if (null != p.getValueAsString()) {
                sb.append(renderValue(p.getValueAsString(), 60));
            } else {
                sb.append("--");
            }
            // Type
            sb.append("</td><td>");
            if (uxTypes.containsValue(p.getClassName())) {
                sb.append(Util.getFirstKeyByValue(uxTypes, p.getClassName()));
            } else {
            ...
            if (p.getFixedValues().isPresent()) {
                for (Object o : p.getFixedValues().get()) {
            ...
            sb.append("<a data-toggle=\"modal\" href=\"#modalEditProperty\" data-pname=\"" + p.getUid() + "\" ");
            ...
            sb.append("?op=" + OP_RMV_PROPERTY + "&" + FEATID + "=" + p.getUid());
    ...
    private static final String renderAuditRows(FF4j ff4j, HttpServletRequest req) {
    	StringBuilder sb = new StringBuilder();
    	ff4j.getAuditTrail().search(new AuditTrailQuery()).forEach(event -> {
    		sb.append("<tr>" + END_OF_LINE);
        	sb.append("<td>" + SDF.format(new Date(event.getTimestamp())) + "</td>");
        	sb.append("<td>" + event.getAction() + "</td>");
        	sb.append("<td>" + event.getUid() + "</td>");
        	sb.append("<td>" + event.getAction() + "</td>");
        	sb.append("</tr>");
		});
    	return sb.toString();
    ...
    private static final String renderFeatureRows(FF4j ff4j, HttpServletRequest req) {
        ...
        ff4j.getRepositoryFeatures().findAll().forEach(f -> {
            ...
            f.getDescription().ifPresent(desc -> sb.append(" tooltip=\"" + desc + "\""));
            sb.append(">");
            sb.append(f.getUid());
            sb.append("</a>");
            // Colonne Group
            ...
            // Colonne Permissions
            sb.append("</td><td>");
            Map < FF4jPermission, FF4jGrantees > permissions = f.getAccessControlList().getPermissions();
            if (null != permissions && !permissions.isEmpty()) {
                boolean first = true;
                for (Entry<FF4jPermission, FF4jGrantees> perm : permissions.entrySet()) {
                    if (!first) {
                        sb.append(",");
                    }
                    sb.append(perm.getKey().name() + "=" + perm.getValue().toJson());
                    first = false;
                }
            } else {
                sb.append("--");
            ...
            f.getToggleStrategies().forEach(fs -> {
                sb.append("<li>" + renderValue(fs.getClass().getName(), 50));
                sb.append("<ol>");
                fs.getProperties().forEach(p -> {
                    sb.append("<li>" + renderValue(p.getUid() + " =  " + p.getValueAsString(), 40));  
                });
                sb.append("</ol>");
            });
            // Colonne 'Holy' Toggle
            ...
            sb.append("<input id=\"" + f.getUid() + "\" type=\"checkbox\" class=\"switch-input\"");
            ...
            // Colonne Button Edit
            sb.append("</td><td style=\"width:5%;text-align:center\">");
            sb.append("<a data-toggle=\"modal\" href=\"#modalEdit\" data-id=\"" + f.getUid() + "\" ");
            sb.append(" data-desc=\"" + f.getDescription() + "\"");
            sb.append(" data-group=\"" + f.getGroup() + "\"");
            ...
            sb.append(req.getContextPath());
            sb.append(req.getServletPath());
            sb.append("?op=" + OP_RMV_FEATURE + "&" + FEATID + "=" + f.getUid());
            sb.append("\" style=\"width:6px;\" class=\"btn\">");
            sb.append("<i class=\"icon-trash\" style=\"margin-left:-5px;\"></i>");
            sb.append("</a>");
            sb.append("</td></tr>");
    ...
    private static String renderGroupList(FF4j ff4j, String modalId) {
        StringBuilder sb = new StringBuilder();
        ff4j.getRepositoryFeatures().listGroupNames().forEach(group -> {
